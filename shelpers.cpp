...
//        head < someFile
        ...
        for(int j = first + 1; j < last; ++j){
            if(tokens[j] == ">" || tokens[j] == "<" ){
                ...
                if(tokens[j] == ">"){
                    if((ret[i].fdStdout = open(tokens[j+1].c_str(), O_CREAT|O_WRONLY|O_TRUNC, 2324 )) < 0){
                        perror("open redirect file failed");
                    }
                    break;
                }
                if(tokens[j] == "<"){
                    if((ret[i].fdStdin = open(tokens[j+1].c_str(), O_RDONLY, 2324)) < 0){
                        perror("open redirect file failed");
                    }
                    //
                    break;
            ...
            //            Implement background commands. If a command runs in the background, the shell should not wait for it              to complete before letting the user enter another command. After the user types a new command and                presses enter, the shell should check to see if any backgrounded commands have completed and an                  informative message saying so. The user indicates that a command should be backgrounded by adding a &            after the command: ./startMyWebServer& #start the webserver, but give me my shell back!:
                ...
                //TO USER the shell while other process is running:
        ...
        if (tokens[last - 1].back() == '&') { // help the & to work without space when perfoming the sleep command:
            ret[i].background = true;
        }
        //Part 3: Adding pipes:
        ...
        if(i > 0){
            ...
            //        cat myFile | head
            //        cat myFile | head -n 100 | tail -n10 #(get the last 10 occurrences of something in myFile)
            //        // define file descriptor
            
            //cat someFile | head -n 100 | tail -n10
            
            int fdPipe [2];
            // if fdPipe is less than zero
            if (pipe(fdPipe) < 0) {
                perror("pipe failed.");
            }
            ret[i].fdStdin = fdPipe[0];
            ret[i-1].fdStdout = fdPipe[1];
